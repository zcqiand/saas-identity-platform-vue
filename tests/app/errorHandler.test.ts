...
describe('global error handler (ch42)', () => {
  ...
  it('installErrorHandler registers app.config.errorHandler', () => {
    const app = createApp({ template: '<div>hi</div>' })
    installErrorHandler(app, { report: (e) => reported.push(e) })
    expect(typeof app.config.errorHandler).toBe('function')
  })

  it('handler captures error + reports to channel', () => {
    const app = createApp({ template: '<div>hi</div>' })
    installErrorHandler(app, { report: (e) => reported.push(e) })
  ...
  it('reportError posts to /api/vitals via mock and resolves (does not throw)', async () => {
