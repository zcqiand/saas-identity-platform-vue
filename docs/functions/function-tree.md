...
| M02 | 角色权限 | 旧版角色 CRUD + 权限矩阵绑定（已迁移到 M00.F03 / M00.F04） | 已废弃 |
| M03 | SSO 登录 | 旧版密码登录 + OIDC + 登出（已迁移到 M01.F04） | 已废弃 |
| M04 | 应用管理 | 应用 / OAuth client 维度：4 F（应用维护 / 应用启用/停用 / 身份认证 / 菜单管理） | 开发中 |
| M05 | API Key | 旧版 API Key 维护（目标 DDL 不再包含 `api_keys`，整体作废） | 已废弃 |
| M06 | 审计 | 旧版审计事件查询 / 留存 / 写入助手（目标 DDL 不再包含 `audit_events`，整体作废） | 已废弃 |
| M08 | 菜单 | 旧版菜单 CRUD + 菜单结构（已迁移到 M04.F04） | 已废弃 |
| M09 | 菜单授权 | 旧版角色菜单授权 + 我的有效菜单（已迁移到 M00.F04 / M04.F04） | 已废弃 |
...
| M04.F04.I08 | 当前用户有效菜单 | 查询 | 前端+后端 | 渲染当前用户在指定应用/租户下有权访问的菜单树 | 开发中 |

---

## M02 角色权限（已废弃）

> 旧版角色 CRUD + 权限矩阵绑定。已迁移至 M00.F03 租户角色 / M00.F04 角色权限，详见 [§0.x 模块重组迁移记录](#0x-模块重组迁移记录2026-09-07)。
> I 级子项见 [§已废弃功能子项](#已废弃功能子项)。

| 功能 ID | 功能名称 | 说明 | 状态 |
|---|---|---|---|
| M02.F01 | 角色 CRUD | 旧版租户作用域角色维护（已迁至 M00.F03） | 已废弃 |
| M02.F02 | 权限矩阵 | 旧版权限码矩阵全量绑定（已迁至 M00.F04.I01） | 已废弃 |

## M03 SSO 登录（已废弃）

> 旧版密码登录 + OIDC + 登出。已迁移至 M01.F04 SSO 登录，详见 [§0.x 模块重组迁移记录](#0x-模块重组迁移记录2026-09-07)。
> I 级子项见 [§已废弃功能子项](#已废弃功能子项)。

| 功能 ID | 功能名称 | 说明 | 状态 |
|---|---|---|---|
| M03.F01 | 密码登录 | 旧版邮箱+密码登录 + 失败锁定（已迁至 M01.F04.I01/I02） | 已废弃 |
| M03.F02 | OIDC | 旧版 authorization_code + refresh_token（已迁至 M01.F04.I04/I05） | 已废弃 |
| M03.F03 | 登出 | 旧版本地清理 + 全局 SSO 登出（已迁至 M01.F04.I06/I07） | 已废弃 |

## M05 API Key（已废弃）

> 旧版 API Key 维护（列表 / 创建 / 吊销 / 轮换 / 物理删除）。目标 DDL 全量切换后不再包含 `api_keys`，整体作废。
> I 级子项见 [§已废弃功能子项](#已废弃功能子项)。

| 功能 ID | 功能名称 | 说明 | 状态 |
|---|---|---|---|
| M05.F01 | API Key 维护 | 旧版 API Key 列表 / 创建 / 吊销 / 轮换 / 物理删除 | 已废弃 |

## M06 审计（已废弃）

> 旧版审计事件查询 / 留存策略 / 写入助手。目标 DDL 全量切换后不再包含 `audit_events`，整体作废。
> I 级子项见 [§已废弃功能子项](#已废弃功能子项)。

| 功能 ID | 功能名称 | 说明 | 状态 |
|---|---|---|---|
| M06.F01 | 事件查询 | 旧版分页查询 / 按用户过滤 / 导出审计事件 | 已废弃 |
| M06.F02 | 留存 | 历史重复编号；目标 DDL 全量切换后作废 | 已废弃 |

## M08 菜单（已废弃）

> 旧版菜单 CRUD + 菜单结构。已迁移至 M04.F04 菜单管理，详见 [§0.x 模块重组迁移记录](#0x-模块重组迁移记录2026-09-07)。
> I 级子项见 [§已废弃功能子项](#已废弃功能子项)。

| 功能 ID | 功能名称 | 说明 | 状态 |
|---|---|---|---|
| M08.F01 | 菜单 CRUD | 旧版菜单列表 / 创建 / 详情 / 更新 / 删除（已迁至 M04.F04.I01-I05） | 已废弃 |
| M08.F02 | 菜单结构 | 旧版同级排序 / 切换父级（已迁至 M04.F04.I06/I07） | 已废弃 |

## M09 菜单授权（已废弃）

> 旧版角色菜单授权 + 我的有效菜单。已迁移至 M00.F04 角色权限 + M04.F04 菜单管理，详见 [§0.x 模块重组迁移记录](#0x-模块重组迁移记录2026-09-07)。
> I 级子项见 [§已废弃功能子项](#已废弃功能子项)。

| 功能 ID | 功能名称 | 说明 | 状态 |
|---|---|---|---|
| M09.F01 | 角色已授权菜单查询 | 旧版角色已授权菜单查询（已迁至 M00.F04.I02） | 已废弃 |
| M09.F02 | 角色菜单写入 | 旧版设置 / 清空角色菜单授权（已迁至 M00.F04.I03/I04） | 已废弃 |
| M09.F03 | 我的有效菜单装配 | 旧版当前用户有效菜单装配（已迁至 M04.F04.I08） | 已废弃 |
...
- 模块已迁：M02 / M03 / M08 / M09 的全部 I 级子项（已废弃，迁至新 ID）
...
| M03.F01.I01 | 密码登录 API | 接口 | 仅后端 | 旧版：邮箱+密码换 session cookie → **M01.F04.I01** | 已废弃 |
| M03.F01.I02 | 失败锁定 | 接口 | 仅后端 | 旧版：连续失败锁定（5 次/15 min） → **M01.F04.I02** | 已废弃 |
| M03.F02.I03 | OIDC Code 换取 | 接口 | 仅后端 | 旧版：authorization_code 换 token → **M01.F04.I04** | 已废弃 |
| M03.F02.I04 | refresh token | 接口 | 仅后端 | 旧版：refresh_token 换新 token → **M01.F04.I05** | 已废弃 |
...
| M08.F01.I01 | 菜单树 | 页面 | 仅前端 | 旧版：菜单树形查询 → **M04.F04.I01** | 已废弃 |
| M08.F01.I02 | 创建菜单 | 按钮 | 仅前端 | 旧版：新建菜单 → **M04.F04.I02** | 已废弃 |
| M08.F01.I03 | 菜单详情 | 页面 | 仅前端 | 旧版：查看菜单 → **M04.F04.I03** | 已废弃 |
| M08.F01.I04 | 更新菜单 | 按钮 | 仅前端 | 旧版：修改菜单字段 → **M04.F04.I04** | 已废弃 |
| M08.F01.I05 | 删除菜单 | 按钮 | 仅前端 | 旧版：删除菜单 → **M04.F04.I05** | 已废弃 |
...
| M09.F02.I03 | 清空角色菜单 | 按钮 | 仅前端 | 旧版：清空角色所有菜单授权 → **M00.F04.I04** | 已废弃 |
| M09.F03.I02 | 角色授权菜单 ID 查询 | 接口 | 前端+后端 | 旧版：membership.roleIds → role_menu_grants.menuIds（装配第一步）→ **M04.F04.I08** | 已废弃 |
| M09.F03.I03 | 菜单树装配 | 接口 | 前端+后端 | 旧版：menuIds → menus 表 + 父链补全（装配第二步）→ **M04.F04.I08** | 已废弃 |
| M09.F03.I04 | 我的有效菜单 | 查询 | 前端+后端 | 旧版：装配可见菜单树 → **M04.F04.I08** | 已废弃 |
